...
void ImageField::addSnapshot(QCustomPlot *Cp, int N)
{
   ...
   QCPRange vis_field_x(0, dim1-1);
   QCPRange vis_field_y(0, dim2-1);
   ...
   double x, y;
   for (int yIndex=0; yIndex<dim1; ++yIndex)
   {
     for (int xIndex=0; xIndex<dim2; ++xIndex)
     {
       colorMap1->data()->cellToCoord(xIndex, yIndex, &x, &y);
       colorMap1->data()->setCell(xIndex, yIndex, ez [yIndex][xIndex]);

       double val_alpha = 255;//0...255 0 full transperent
